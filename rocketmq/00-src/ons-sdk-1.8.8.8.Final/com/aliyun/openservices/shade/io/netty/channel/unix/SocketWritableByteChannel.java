...
public abstract class SocketWritableByteChannel implements WritableByteChannel {
    ...
    public final int write(java.nio.ByteBuffer src) throws java.io.IOException {
        ...
        if (src.isDirect()) {
            ...
            com.aliyun.openservices.shade.io.netty.buffer.ByteBuf buffer = null;
            try {
                if (readableBytes == 0) {
                    buffer = com.aliyun.openservices.shade.io.netty.buffer.Unpooled.EMPTY_BUFFER;
                } else {
                    final ByteBufAllocator alloc = alloc();
                    if (alloc.isDirectBufferPooled()) {
                        buffer = alloc.directBuffer(readableBytes);
                    } else {
                        buffer = com.aliyun.openservices.shade.io.netty.buffer.ByteBufUtil.threadLocalDirectBuffer();
                        if (buffer == null) {
                            buffer = com.aliyun.openservices.shade.io.netty.buffer.Unpooled.directBuffer(readableBytes);
                        }
                    }
                }
